...
const activityLogs = [];
// Function to fetch todos from local storage.
function fetchactivitylogFromLocalStorage() {
  const storedlog = localStorage.getItem('activityLogs');
  if (storedlog) {
    todos = JSON.parse(storedlog);
  }
}
...
function getDueDate(dueDateInput) {
    ...
    if (dueDateInput.toLowerCase().includes('tomorrow')) {
      ...
      flag=true;
    } else if(dueDateInput.toLowerCase().includes('today')){
      flag=true;
    }
    ...
    datedue=flag==true?currentDate:dateString;
    // const dueDate = new Date(dateString);
    const dueDate = new Date(datedue);
    console.log(dueDate,"123");
  ...
  function displayTodos() {
    ...
    todos.forEach(todo => {
      ...
      todoTextElement.className= 'main_task';
      ...
      todoItem.appendChild(categoryElement);
        ...
        // Due Date label and input field
      const dueDateLabel = document.createElement('label');
      dueDateLabel.textContent = 'Due Date:';
      const dueDateSpan = document.createElement('span');
      dueDateSpan.className='dueDate';
      dueDateSpan.textContent = new Date(todo.dueDate).toLocaleString();
    ...
    addSubtaskButton.className= 'subtask_button';
    ...
    if(todo.subtasks.length>0){
      ...
      toggleSubtasksButton.className= 'subtask_button';
      ...
      if(todo.showSubtasks){
        ...
        todo.subtasks.forEach((subtask,sub_index)=>{
            // console.log("display",subtask);
            const subtaskItem=document.createElement('li');
            subtaskItem.className='subtask-item';

            //to make subtaks draggable
          subtaskItem.draggable = true;

          //drag events
          subtaskItem.addEventListener('dragstart', event => {
             
            draggedSubtaskIndex = sub_index;
            draggedTodoIndex = todo.index;
            
            // console.log('123',sub_index);
          });
          ...
          subtaskItem.addEventListener('drop', event => {
            event.preventDefault();
            // Get the index of the drop target subtask item
            const dropTargetIndex = sub_index;

            // Re-arrange the subtasks within the parent todo and update the display
            if (draggedSubtaskIndex !== dropTargetIndex) {
              const [draggedSubtask] = todos[draggedTodoIndex].subtasks.splice(draggedSubtaskIndex, 1);
              todos[draggedTodoIndex].subtasks.splice(dropTargetIndex, 0, draggedSubtask);
              draggedSubtaskIndex = dropTargetIndex;
              saveTodosToLocalStorage();
              displayTodos(); // Update the display after re-arranging
            }
          });
            ...
            subtaskEditButton.textContent='✎';
            subtaskEditButton.title='Edit';
            subtaskEditButton.style.backgroundColor = '#08770d'
            ...
            subtaskDeleteButton.textContent='🗑️';
            subtaskDeleteButton.title='Delete';
            subtaskDeleteButton.style.backgroundColor = '#bb1919'
   ...
   todos.forEach((todo, index) => {
    const todoItem = todoListContainer.children[index];
    ...
    todoItem.addEventListener('drop', event => {
      event.preventDefault();
      // Get the index of the drop target todo item
      const dropTargetIndex = index;

       
      // Re-arrange the todos array and update the display
      if (draggedTodoIndex !== dropTargetIndex) {
        const [draggedTodo] = todos.splice(draggedTodoIndex, 1);
        todos.splice(dropTargetIndex, 0, draggedTodo);
        draggedTodoIndex = dropTargetIndex;
        saveTodosToLocalStorage();
        displayTodos();  
      }
    });
...
function performSearch() {
  ...
  function sortByDueDate(mark=true) {
    let sortedTodos;
    if(mark){
    sortedTodos = todos.slice().sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate));
    }
    else{
      sortedTodos = todos.slice().sort((a, b) => new Date(b.dueDate) - new Date(a.dueDate));
    }
    return sortedTodos;
  }
  
  // Function to sort todos by priority
  function sortByPriority(mark=true) {
    let sortedTodos;
    if(mark){
    ...
    sortedTodos = todos.slice().sort((a, b) => {
      const priorityOrder1 = { 'low': 3, 'medium': 2, 'high': 1 };
      return priorityOrder1[a.priority] - priorityOrder1[b.priority];
    });
...
//   viewBacklogs section
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (sortBy === 'dueDateasc') {
      sortedTodos = sortByDueDate(true);
    } else if (sortBy === 'dueDatedsc') {
      sortedTodos = sortByDueDate(false);
    }else if (sortBy === 'prioritylowtohigh') {
      sortedTodos = sortByPriority(true);
    }else if (sortBy === 'priorityhightolow') {
      sortedTodos = sortByPriority(false);
    }
